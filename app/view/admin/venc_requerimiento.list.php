...
<script type="text/javascript">
	...
	function Alerta(ID){
		$.getJSON('<?php echo $URL_ROOT;?>ajax/alert_requerimiento.php?requerimientoID='+ID, function(data) {
			if(data.retval==1){
...
<section class="tables">   
						...
						<div class="card-body">
							<div class="box-body">
								<table class="table table-bordered table-hover" width='100%' id="dataTables-example">
									...
									<tbody>
										<?php $list=CrmRequerimiento::getList_Paging(); foreach ($list as $oItem) { 

											/*Proveedor */ $oProveedor = CrmProveedor::getItem($oItem->proveedorID);
											/*PropxForm */ $oCrmPropxForm = CrmPropxForm::getItem($oItem->propxformID);
											...
											?>
											<tr>
												<td align="center"><?php echo $oItem->requerimientoID; ?></td>
												...
												<td align="center" <?php if($oItem->state == 1 || $oItem->state == 2){ echo 'style="background-color: greenyellow;"'; }else{echo 'style="background-color: red;"'; } ?> ><?php echo CrmRequerimiento::getState($oItem->state);?></td>
												<td><a href="<?php echo "javascript:ModalDate(".$oItem->requerimientoID.",'".$oItem->registerDate."','".$oItem->registerExpire."');"; ?>"><i class="fa fa-plus"></i></a>&nbsp;&nbsp;<a href="<?php echo "javascript:Alerta(".$oItem->requerimientoID.");"; ?>"><i class="fa fa-bell"></i></a></td>
											</tr>
											<?php } ?>
										</tbody>
									</table>
		...
		<div class="modal-dialog modal-lg" role="document" >
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="gridSystemModalLabel">Cambiar Plazo de Requerimiento</h4>
				...
				<div class="modal-footer">
					<div class="row">
						<div class="col-sm-5">
							<div class="btn btn-primary btn-block" name="btnUpdateRequerimiento" id="btnUpdateRequerimiento"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Guardar</div>
						</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#btnUpdateRequerimiento').click(function(){
				var requerimientoHidID      			= $("#requerimientoHidID").val();
				var registerExpire       		 		= $("#registerExpire").val();		
				$.getJSON('<?php echo $URL_ROOT;?>ajax/update_date.php?requerimientoID='+requerimientoHidID+'&registerExpire='+registerExpire, function(data) {
					if(data.retval==1){
						...
						alertify.error('No se pudo Actualizar el requerimiento , Contactarse con Soporte - BV');
					}
