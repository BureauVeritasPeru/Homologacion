...
class AdmUser extends Database
{
    ...
    public static function  getItem($userID){
        $query = "
        SELECT adm.userID,  adm.firstName, adm.lastName, adm.userName, adm.password, adm.email, adm.profileID, adm.state, adm.isDefault, prof.profileName,adm.online
    ...
    public static function  getListHomologador(){
        $list=new Collection();
        $query ="
        SELECT adm.userID, adm.firstName, adm.lastName
        FROM adm_user AS adm
        WHERE adm.profileID = '2'
        AND adm.state = '1'
        ";
